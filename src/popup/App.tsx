...
function App() {
  ...
  const handleBookmark = async () => {
    try {
      ...
      logger.info('App', 'Creating bookmark', { url: currentUrl });
      ...
      if (isBookmarked) {
        ...
        logger.info('App', 'Bookmark removed');
        alert('Bookmark removed!');
      } else {
        // Create bookmark on homeserver using SDK
        const pubkyUrl = await pubkyAPISDK.createBookmark(currentUrl);
        ...
        logger.info('App', 'Bookmark created successfully');
        alert('Bookmarked!');
      }
    } catch (error) {
      logger.error('App', 'Failed to bookmark', error as Error);
